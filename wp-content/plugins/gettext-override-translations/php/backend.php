...
class MP_Gettext_Override_Admin {
	...
	{
		define('DC_PLUGIN_URL', WP_CONTENT_URL . '/plugins/gettext-override-translations/');
		define('DC_PLUGIN_DIR', WP_PLUGIN_DIR . '/gettext-override-translations/');
	...
	{
		$output = str_replace('<script', '', $input); // We allow HTML, but strip script
		return $output;
	}
	...
	?>
			<?php $constants = get_option(MP_GETTEXT_OVERRIDE_OPTION); 
				...
				<form method="post" id="dc_defined_constants_form" action="options.php">
					...
					<?php foreach($constants as $key => $value) : ?>
						<tr valign="top"<?php if(isset($value['internal_warning']) && $value['internal_warning'] == 1) echo ' class="internal_warning" ';?>>
							...
							<td>
								<textarea cols="40" rows="5" name="defined_constants[name][]"><?php if(isset($value['name'])) echo $value['name']; ?></textarea>
							</td>
